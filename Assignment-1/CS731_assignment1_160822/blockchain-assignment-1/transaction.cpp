...
bool txn_t::validate() 
{
    ...
    rsa_public_key_t pubkey1(public_key.data(), public_key.size());
    uint8_t h1[SHA256_DIGEST_LENGTH], h2[SHA256_DIGEST_LENGTH];
    sha256((const uint8_t*)public_key.data(),public_key.size(),const_cast<uint8_t*>(h1));
    for(unsigned i=0; i!=SHA256_DIGEST_LENGTH; i++){
        if(h1[i]!=source_addr[i])
            return false;
    }
    ...
    SHA256_Update(&sha256, change_addr.data(), change_addr.size());
    SHA256_Update(&sha256, &amount, sizeof(amount));
    SHA256_Final(const_cast<uint8_t*>(h2), &sha256);
    for(unsigned i=0; i!=SHA256_DIGEST_LENGTH; i++){
        if(h2[i]!=tx_hash[i])
            return false;
    }
    
    bool ans = pubkey1.verify((const uint8_t*)tx_hash.data(),tx_hash.size(),tx_sign.data(),tx_sign.size());
